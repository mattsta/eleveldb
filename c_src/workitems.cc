...
WorkTask::WorkTask(ErlNifEnv *caller_env, ERL_NIF_TERM &caller_ref)
    ...
    if (NULL != caller_env) {
        ...
        caller_pid_term =
            enif_make_pid(local_env_, enif_self(caller_env, &local_pid));
        terms_set = true;
    } // if
    else {
        local_env_ = NULL;
...
void WorkTask::prepare_recycle() {
    // does not work by default
    resubmit_work = false;
} // WorkTask::prepare_recycle

void WorkTask::recycle() {
    // does not work by default
} // WorkTask::recycle
...
OpenTask::OpenTask(ErlNifEnv *caller_env, ERL_NIF_TERM &_caller_ref,
                   const std::string &db_name_, leveldb::Options *open_options_)
    : WorkTask(caller_env, _caller_ref), db_name(db_name_),
      open_options(open_options_) {} // OpenTask::OpenTask

work_result OpenTask::operator()() {
    void *db_ptr_ptr;
    leveldb::DB *db(0);
...
work_result MoveTask::operator()() {
    ...
    default:
        ...
        // JDB: note: We can't send an exception as a message. It crashes
        // Erlang.
    ...
    // debug syslog(LOG_ERR, "                     MoveItem::operator() %d, %d,
    // %d",
...
void MoveTask::prepare_recycle() {

    resubmit_work = true;

} // MoveTask::prepare_recycle

void MoveTask::recycle() {
