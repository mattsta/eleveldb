...
class RefObject {
  ...
  protected:
    volatile uint32_t
    m_RefCount; //!< simple count of reference, auto delete at zero
...
template <class TargetT> class ReferencePtr {
    TargetT *t;

  public:
    ReferencePtr() : t(NULL) {};

    ReferencePtr(TargetT *_t) : t(_t) {
        if (NULL != t)
            t->RefInc();
    }
...
class DbObject : public ErlRefObject {
  public:
    ...
    std::list<class ItrObject *>
    m_ItrList; //!< ItrObjects holding ref count to this
  ...
  public:
    ...
    static DbObject *RetrieveDbObject(ErlNifEnv *Env,
...
class LevelIteratorWrapper : public RefObject {
  public:
    ReferencePtr<DbObject>
    m_DbPtr; //!< need to keep db open for delete of this object
    ReferencePtr<class ItrObject>
    m_ItrPtr; //!< shared itr_ref requires we hold ItrObject
    const leveldb::Snapshot *m_Snapshot;
    leveldb::Iterator *m_Iterator;
    volatile uint32_t
    m_HandoffAtomic; //!< matthew's atomic foreground/background prefetch flag.
    ...
    bool Valid() {
        return (NULL != m_Iterator && m_Iterator->Valid());
    };
    leveldb::Slice key() {
  ...
  private:
    ...
    LevelIteratorWrapper &
    operator=(const LevelIteratorWrapper &); // no assignment
...
class ItrObject : public ErlRefObject {
  public:
    ...
    bool keys_only;
    leveldb::ReadOptions
    m_ReadOptions; //!< local copy, pass to LevelIteratorWrapper only

    volatile class MoveTask *reuse_move; //!< iterator work object that is
                                         //reused instead of lots malloc/free
  ...
  public:
    ...
    static void *CreateItrObject(DbObject *Db, bool KeysOnly,
                                 leveldb::ReadOptions &Options);

    static ItrObject *RetrieveItrObject(ErlNifEnv *Env,
                                        const ERL_NIF_TERM &DbTerm,
