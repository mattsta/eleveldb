...
class WorkTask : public RefObject {
  ...
  public:
    ...
    const ERL_NIF_TERM &caller_ref() {
        local_env();
        return caller_ref_term;
    }
    const ERL_NIF_TERM &pid() {
        local_env();
        return caller_pid_term;
    }
...
class OpenTask : public WorkTask {
  protected:
    std::string db_name;
    leveldb::Options *
    open_options; // associated with db handle, we don't free it
...
class WriteTask : public WorkTask {
  ...
  public:
    WriteTask(ErlNifEnv *_owner_env, ERL_NIF_TERM _caller_ref,
              DbObject *_db_handle, leveldb::WriteBatch *_batch,
              leveldb::WriteOptions *_options)
        : WorkTask(_owner_env, _caller_ref, _db_handle), batch(_batch),
          options(_options) {}
...
class GetTask : public WorkTask {
  ...
  public:
    GetTask(ErlNifEnv *_caller_env, ERL_NIF_TERM _caller_ref,
            DbObject *_db_handle, ERL_NIF_TERM _key_term,
            leveldb::ReadOptions &_options)
        : WorkTask(_caller_env, _caller_ref, _db_handle), options(_options) {
        ErlNifBinary key;
...
class IterTask : public WorkTask {
  ...
  public:
    IterTask(ErlNifEnv *_caller_env, ERL_NIF_TERM _caller_ref,
             DbObject *_db_handle, const bool _keys_only,
             leveldb::ReadOptions &_options)
        : WorkTask(_caller_env, _caller_ref, _db_handle), keys_only(_keys_only),
          options(_options) {}
    ...
    virtual work_result operator()() {
        ...
        itr_ptr_ptr =
            ItrObject::CreateItrObject(m_DbPtr.get(), keys_only, options);
        ...
        itr_ptr->m_Iter.assign(new LevelIteratorWrapper(
            itr_ptr, keys_only, options, itr_ptr->itr_ref));
...
class MoveTask : public WorkTask {
  ...
  protected:
    ReferencePtr<LevelIteratorWrapper>
    m_ItrWrap; //!< access to database, and holds reference
  ...
  public:
    ...
    MoveTask(ErlNifEnv *_caller_env, ERL_NIF_TERM _caller_ref,
        ...
        : WorkTask(NULL, _caller_ref, IterWrap->m_DbPtr.get()),
          m_ItrWrap(IterWrap), action(_action), seek_target(_seek_target) {
        // special case construction
        local_env_ = NULL;
        enif_self(_caller_env, &local_pid);
    }
...
class ItrCloseTask : public WorkTask {
  ...
  public:
    ItrCloseTask(ErlNifEnv *_owner_env, ERL_NIF_TERM _caller_ref,
        ...
        : WorkTask(_owner_env, _caller_ref), m_ItrPtr(_itr_handle) {}
    ...
    virtual work_result operator()() {
        ...
        if (NULL != itr_ptr) {
            ...
            //            return work_result(local_env(), ATOM_ERROR,
            // ATOM_BADARG);
            return (work_result()); // no message
        }                           // else
    }
