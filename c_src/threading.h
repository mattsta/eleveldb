...
class eleveldb_thread_pool {
  ...
  private:
    ...
    operator=(const eleveldb_thread_pool &); // nocopyassign
  ...
  private:
    ...
    volatile size_t
    work_queue_atomic; //!< atomic size to parallel work_queue.size().
  ...
  public:
    ...
    leveldb::PerformanceCounters *perf() const {
        return (leveldb::gPerfCounters);
    };
