...
struct ThreadData {
    ...
    volatile uint32_t m_Available; //!< 1 if thread waiting, using standard type
...
bool // returns true if available worker thread found and claimed
    eleveldb_thread_pool::FindWaitingThread(
        eleveldb::WorkTask *
            work) // non-NULL to pass current work directly to a thread,
// NULL to potentially nudge an available worker toward backlog queue
{
    ...
    do {
        ...
        if (0 != threads[index]->m_Available) {
            ...
            ret_flag =
                eleveldb::compare_and_swap(&threads[index]->m_Available, 1, 0);
...
eleveldb_thread_pool::eleveldb_thread_pool(const size_t thread_pool_size)
    : work_queue_pending(0), work_queue_lock(0), work_queue_atomic(0),
      shutdown(false) {

    work_queue_pending =
        enif_cond_create(const_cast<char *>("work_queue_pending"));
...
bool eleveldb_thread_pool::grow_thread_pool(const size_t nthreads) {
    ...
    for (size_t i = nthreads; i; --i) {
        ...
        ErlNifTid *thread_id =
            static_cast<ErlNifTid *>(enif_alloc(sizeof(ErlNifTid)));
        ...
        const int result = enif_thread_create(
            const_cast<char *>(thread_name.str().c_str()), thread_id,
            eleveldb_write_thread_worker, static_cast<void *>(new_thread), 0);
...
bool eleveldb_thread_pool::notify_caller(eleveldb::WorkTask &work_item) {
    ...
    if (result.is_set()) {
        if (0 !=
            enif_get_local_pid(work_item.local_env(), work_item.pid(), &pid)) {
            /* Assemble a notification of the following form:
               { PID CallerHandle, ERL_NIF_TERM result } */
            ERL_NIF_TERM result_tuple = enif_make_tuple2(
                work_item.local_env(), work_item.caller_ref(), result.result());

            ret_flag =
                (0 != enif_send(0, &pid, work_item.local_env(), result_tuple));
        } // if
        else {
            ret_flag = false;
        } // else
 ...
 *  B. processing work passed by Erlang thread: m_Available=0,
 * m_DirectWork=<non-null>
...
void *eleveldb_write_thread_worker(void *args) {
    ...
    while (!h.shutdown) {
        ...
        //  (but retest queue before sleep due to race condition)
            ...
            submission =
                (eleveldb::WorkTask *)tdata.m_DirectWork; // NULL is valid
            tdata.m_DirectWork = NULL;                    // safety
