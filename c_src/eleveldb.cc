...
static ERL_NIF_TERM error_tuple(ErlNifEnv *env, ERL_NIF_TERM error,
                                leveldb::Status &status) {
    ERL_NIF_TERM reason =
        enif_make_string(env, status.ToString().c_str(), ERL_NIF_LATIN1);
...
struct EleveldbOptions {
    ...
    EleveldbOptions()
        : m_EleveldbThreads(71), m_LeveldbImmThreads(0),
          m_LeveldbBGWriteThreads(0), m_LeveldbOverlapThreads(0),
          m_LeveldbGroomingThreads(0), m_TotalMemPercent(0), m_TotalMem(0),
...
ERL_NIF_TERM parse_init_option(ErlNifEnv *env, ERL_NIF_TERM item,
                               EleveldbOptions &opts) {
    int arity;
        ...
        if (option[0] == eleveldb::ATOM_TOTAL_LEVELDB_MEM) {
            ...
            if (enif_get_ulong(env, option[1], &memory_sz)) {
                if (0 < memory_sz && memory_sz <= 100) {
                    // this gets noticed later and applied against
                    // gCurrentTotalMemory
...
ERL_NIF_TERM parse_open_option(ErlNifEnv *env, ERL_NIF_TERM item,
    ...
    if (enif_get_tuple(env, item, &arity, &option) && 2 == arity) {
        if (option[0] == eleveldb::ATOM_CREATE_IF_MISSING)
            ...
            if (enif_get_ulong(env, option[1], &sst_block_sz))
                opts.block_size = sst_block_sz; // Note: We just set the "old"
                                                // block_size option.
            ...
            // per-table basis. We only disable it if explicitly asked.
            // Alternatively,
            // one can provide a value for # of bits-per-key.
            unsigned long bfsize = 16;
            if (option[1] == eleveldb::ATOM_TRUE ||
                enif_get_ulong(env, option[1], &bfsize)) {
            ...
            ret_val =
                enif_get_string(env, option[1], buffer, 256, ERL_NIF_LATIN1);
...
ERL_NIF_TERM write_batch_item(ErlNifEnv *env, ERL_NIF_TERM item,
    ...
    if (enif_get_tuple(env, item, &arity, &action) || enif_is_atom(env, item)) {
...
ERL_NIF_TERM send_reply(ErlNifEnv *env, ERL_NIF_TERM ref, ERL_NIF_TERM reply) {
    ...
    ERL_NIF_TERM msg = enif_make_tuple2(msg_env, enif_make_copy(msg_env, ref),
...
ERL_NIF_TERM
async_open(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    char db_name[4096];

    if (!enif_get_string(env, argv[1], db_name, sizeof(db_name),
                         ERL_NIF_LATIN1) ||
    ...
    //  This happens now because there is no guarantee as to when the
    // total_memory
    //  value would be read relative to total_leveldb_mem_percent in the option
    // fold
    uint64_t use_memory;
    ...
    if (0 < priv.m_Opts.m_TotalMemPercent &&
        priv.m_Opts.m_TotalMemPercent <= 100)
        use_memory = (priv.m_Opts.m_TotalMemPercent * use_memory) /
                     100; // integer math for percentage
    ...
    eleveldb::WorkTask *work_item =
        new eleveldb::OpenTask(env, caller_ref, db_name, opts);
...
ERL_NIF_TERM
async_write(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    if (NULL == db_ptr.get() || !enif_is_list(env, action_ref) ||
        ...
        return send_reply(
            ...
            enif_make_tuple3(env, eleveldb::ATOM_ERROR, caller_ref,
                             enif_make_tuple2(env,
                                              eleveldb::ATOM_BAD_WRITE_ACTION,
                                              result)));
    } // if
    ...
    eleveldb::WorkTask *work_item =
        new eleveldb::WriteTask(env, caller_ref, db_ptr.get(), batch, opts);
...
ERL_NIF_TERM
async_get(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    const ERL_NIF_TERM &caller_ref = argv[0];
    ...
    if (NULL == db_ptr.get() || !enif_is_list(env, opts_ref) ||
    ...
    eleveldb::WorkTask *work_item =
        new eleveldb::GetTask(env, caller_ref, db_ptr.get(), key_ref, opts);
...
ERL_NIF_TERM
async_iterator(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    const ERL_NIF_TERM &caller_ref = argv[0];
    const ERL_NIF_TERM &dbh_ref = argv[1];
    ...
    eleveldb::WorkTask *work_item =
        new eleveldb::IterTask(env, caller_ref, db_ptr.get(), keys_only, opts);
    ...
    if (false == priv.thread_pool.submit(work_item)) {
        ...
        return send_reply(env, caller_ref,
                          enif_make_tuple2(env, ATOM_ERROR, caller_ref));
    } // if
...
ERL_NIF_TERM
async_iterator_move(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    // const ERL_NIF_TERM& caller_ref       = argv[0];
    ...
    /* We can be invoked with two different arities from Erlang. If our
       "action_atom" parameter is not
       in fact an atom, then it is actually a seek target. Let's find out which
       we are: */
    eleveldb::MoveTask::action_t action = eleveldb::MoveTask::SEEK;

    // If we have an atom, it's one of these (action_or_target's value is
    // ignored):
    if (enif_is_atom(env, action_or_target)) {
        if (ATOM_FIRST == action_or_target)
            action = eleveldb::MoveTask::FIRST;
        if (ATOM_LAST == action_or_target)
            action = eleveldb::MoveTask::LAST;
        if (ATOM_NEXT == action_or_target)
            action = eleveldb::MoveTask::NEXT;
        if (ATOM_PREV == action_or_target)
            action = eleveldb::MoveTask::PREV;
        if (ATOM_PREFETCH == action_or_target)
            action = eleveldb::MoveTask::PREFETCH;
        if (ATOM_PREFETCH_STOP == action_or_target)
            action = eleveldb::MoveTask::PREFETCH_STOP;
    } // if

    // debug syslog(LOG_ERR, "move state: %d, %d, %d",
    //              action, itr_ptr->m_Iter->m_PrefetchStarted,
    // itr_ptr->m_Iter->m_HandoffAtomic);
    ...
    itr_ptr->m_Iter->m_PrefetchStarted =
        prefetch_state && (eleveldb::MoveTask::PREFETCH_STOP != action);
    ...
    //  prefetch waiting for us
    else if (eleveldb::compare_and_swap(&itr_ptr->m_Iter->m_HandoffAtomic, 0,
                                        1)) {
        ...
        // is this truly a wait for prefetch ... or actually the first prefetch
        // request
        if (!prefetch_state) {
            submit_new_request = true;
            itr_ptr->ReleaseReuseMove();
    ...
    // case #3
        ...
        if (!itr_ptr->m_Iter->Valid())
            ...
            ret_term = enif_make_tuple2(
                env, ATOM_OK, slice_to_binary(env, itr_ptr->m_Iter->key()));
        else
            ret_term = enif_make_tuple3(
                env, ATOM_OK, slice_to_binary(env, itr_ptr->m_Iter->key()),
...
ERL_NIF_TERM
async_close(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    const ERL_NIF_TERM &caller_ref = argv[0];
    const ERL_NIF_TERM &dbh_ref = argv[1];
    bool term_ok = false;
    ...
    if (NULL != db_ptr->m_Db
                //        && compare_and_swap(db_ptr->m_ErlangThisPtr,
                // db_ptr.get(), (DbObject *)NULL))
        &&
        db_ptr->ClaimCloseFromCThread()) {
        eleveldb::WorkTask *work_item =
            new eleveldb::CloseTask(env, caller_ref, db_ptr.get());
...
ERL_NIF_TERM
async_iterator_close(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    const ERL_NIF_TERM &caller_ref = argv[0];
    ...
    //    if (compare_and_swap(itr_ptr->m_ErlangThisPtr, itr_ptr.get(),
    // (ItrObject *)NULL))
    if (itr_ptr->ClaimCloseFromCThread()) {
        eleveldb::WorkTask *work_item =
            new eleveldb::ItrCloseTask(env, caller_ref, itr_ptr.get());
...
ERL_NIF_TERM
eleveldb_status(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ErlNifBinary name_bin;
    eleveldb::ReferencePtr<eleveldb::DbObject> db_ptr;

    db_ptr.assign(eleveldb::DbObject::RetrieveDbObject(env, argv[0]));

    if (NULL != db_ptr.get() && enif_inspect_binary(env, argv[1], &name_bin)) {
        if (db_ptr->m_Db == NULL) {
        ...
        if (db_ptr->m_Db->GetProperty(name, &value)) {
            ...
            unsigned char *result_buf =
                enif_make_new_binary(env, value.size(), &result);
...
ERL_NIF_TERM
eleveldb_repair(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    char name[4096];
 ...
 */
ERL_NIF_TERM
eleveldb_destroy(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    char name[4096];
    if (enif_get_string(env, argv[0], name, sizeof(name), ERL_NIF_LATIN1) &&
...
ERL_NIF_TERM
eleveldb_is_empty(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    eleveldb::ReferencePtr<eleveldb::DbObject> db_ptr;
...
#undef ATOM
    ...
    /* Refuse to load the NIF module (I see no way right now to return a more
    specific exception
